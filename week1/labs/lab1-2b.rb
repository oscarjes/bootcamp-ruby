...
# puts "oscar's example - capitalized names:"
# names = ["adam", "bob", "charles"]
# names.map {|n| n.capitalize}.each_with_index do |line, index|
#    puts "Line #{index + 1} :: #{line} :: #{line.split.length} words"
# end
...
# puts "oscar's example - candy ranking"
# ranking = ["kit kat", "mars", "snickers", "oreos"].map.with_index {|c, index| "#{index+1} - #{c}"}
# puts ranking
